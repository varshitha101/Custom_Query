...
export default async function handleQueryFetch(expression, expressionString, setIsLoading, setRecivedLength, setIsReciving, setProcessingNode) {
  ...
  try {
    ...
    const response = await axios.post(`${import.meta.env.VITE_BASE_SERVER_URL}/query/fetch/V2`, { expression, expressionString });
    console.log("response", response);
    let excelResponse = response.data;
    console.log("Initial excelResponse length:", Array.isArray(excelResponse) ? excelResponse.length : "Not an array");
    console.log("type of excelResponse:", typeof excelResponse);
    if (!excelResponse) {
    ...
    if (excelResponse.length > 0) {
      const excelData = excelResponse.map((row) => {
        ...
        return {
          ...
          "Fagerstorm Test for Nicotine Dependence - Do you smoke more frequently during the first hours of waking than during the rest of the day?": mapping(row.tcc_form[tcclastKey]?.de) || "",
    ...
    console.log(`Query fetch took ${(endTime - startTime).toFixed(2)} milliseconds.`);
    setIsLoading(false);
    setIsReciving(false);
  }
}
