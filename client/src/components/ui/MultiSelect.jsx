...
export default function MultiSelect({ value, options, onChange, inputValue, onInputChange, error, helperText, maxSelections, getOptionDisabled }) {
  ...
  const isSelectionAtLimit = typeof maxSelections === "number" && value.length >= maxSelections;
  ...
  return (
    <Autocomplete
      ...
      renderInput={(params) => <TextField {...params} label="Select Multiple" sx={styles} error={!!error} helperText={error || helperText} />}
    />
  );
}
