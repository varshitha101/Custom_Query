...
export default function OptionSelector({ id, data, onDelete, errorSelectors, selectors, setSelectors, onAdd, setExpression, isDateSelect }) {
  ...
  const panchayathSelectorCount = selectedOptions2.filter((option) => option === "Panchayath").length;
  ...
  if (selector?.selectedOption2 !== "Panchayath" && panchayathSelectorCount > 0) disabledOptions.push("Panchayath");
  ...
  const handleSelectedOption1Change = useCallback(
    (event, newSelectedOption1) => {
      if (isMandatoryPanchayathSelector) {
    ...
    [id, isMandatoryPanchayathSelector, setExpression, setSelectors],
  );
  ...
  const handleSelectedOption2Change = useCallback(
    (event, newSelectedOption2) => {
      if (isMandatoryPanchayathSelector && newSelectedOption2 !== "Panchayath") {
        toast.error("At least one Panchayath selector is required");
        return;
      }
  ...
  const handleSelectedOption3Change = useCallback(
    (eOrDate, newValOrType) => {
      ...
      if (selector?.selectedOption2 === "Panchayath" && Array.isArray(newVal) && newVal.length > 5) {
        toast.error("Only 5 Panchayaths can be selected");
        return;
      }
