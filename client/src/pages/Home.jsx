...
export default function Home() {
  ...
  const defaultGeneralOption = data.find((option) => option.msg === "General") ?? null;
  ...
  const [selectors, setSelectors] = useState(() => [
    {
      id: 0,
      selectedOption1: defaultGeneralOption,
      selectedOption2: "Panchayath",
      selectedOption3: [],
    },
  ]);
  ...
  const nodeVlaue = {
    ...
    Form_3: "Form 3",
    tcc_form: "TCC Form",
    profile_history1: "Profile History",
    general: "Form 1",
  };

  const isOption3Missing = useCallback((selector) => {
    if (selector.selectedOption2 === "Date") {
      return !selector.selectedOption3?.SDate || !selector.selectedOption3?.LDate;
    }

    if (multiSelectFields.includes(selector.selectedOption2)) {
      return !Array.isArray(selector.selectedOption3) || selector.selectedOption3.length === 0;
    }

    if (rangeSelectFields.includes(selector.selectedOption2)) {
      return !selector.selectedOption3?.operator || (!selector.selectedOption3?.name && !selector.selectedOption3?.id);
    }

    if (rangeSelectPlusEnterFields.includes(selector.selectedOption2)) {
      return !selector.selectedOption3?.operator || selector.selectedOption3?.value === undefined || selector.selectedOption3.value === "";
    }

    if (typeof selector.selectedOption3 === "string") {
      return selector.selectedOption3.trim() === "";
    }

    if (selector.selectedOption3 && typeof selector.selectedOption3 === "object") {
      if ("value" in selector.selectedOption3) {
        return selector.selectedOption3.value === undefined || selector.selectedOption3.value === "";
      }

      return !selector.selectedOption3.name && !selector.selectedOption3.id;
    }

    return selector.selectedOption3 == null;
  }, []);
  ...
  const handleDeleteSelector = useCallback(
    (id) => {
      ...
      const selectorToDelete = selectors.find((sel) => sel.id === id);
      const panchayathSelectorCount = selectors.filter((sel) => sel.selectedOption2 === "Panchayath").length;

      if (selectorToDelete?.selectedOption2 === "Panchayath" && panchayathSelectorCount === 1) {
        toast.error("At least one Panchayath selector is required");
        return;
      }
  ...
  const errorChecker = useCallback(() => {
    ...
    const newErrorSelectors = selectors
      .map((s) => {
        const selectedOptions = [s.selectedOption1 === null, s.selectedOption2 === null, isOption3Missing(s)];
      ...
      <Modal open={isLoading}>
        <ModalDialog variant="outlined">
          <DialogContent sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <CircularProgress /> {processingNode === "" ? "Loading..." : `Processing ${nodeVlaue[processingNode] ?? processingNode}`}
          </DialogContent>
        </ModalDialog>
      </Modal>
